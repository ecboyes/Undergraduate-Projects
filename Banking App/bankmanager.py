...
class BankManager(EasyFrame):
    ...
    def create_widgets(self):
        ...
        self.nextButton = self.addButton(row = 4, column = 2,
                                         text = "Find account",
                                         command = self.nextAccount)
        self.saveButton = self.addButton(row = 4, column = 3,
                                         text = "Save to file",
    ...
    def findAccount(self):
        self.prompterBox(title = "Find Account", promptString = "None",
                         inputText = "", fieldWidth = 20)
        self.prompterBox(title = "Find Account", promptString = "Pin",
                         inputText = "", fieldWidth = 20)
        key = self.bank.makeKey(name, pin)
        keyList = self.bank.getKeys()
        if key in bank.getKey:
            self.account = self.bank.getKeys().index(self.bank.makeKey(name, pin))
            self.position = self.updateAccount()
            self.displayAccount()
        else:
            self.statusField.setText("No such account")
